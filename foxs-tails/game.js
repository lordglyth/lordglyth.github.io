(() => {
  ...
  const ribbons = [
    ...
    {id:'moon', name:'shrine', map:'shrine', x:840, y:1000, color:'#9fdcff', display:'Moon Ribbon'}
  ];
  ...
  const MAPS = {
    village: {
      ...
      portals:[
        {id:'to-moss',x:40,y:1020,w:100,h:250,to:'mosswood',spawn:{x:1750,y:760},label:'Mosswood Trail',dir:'left'},
        {id:'to-shrine',x:1660,y:500,w:100,h:360,to:'shrine',spawn:{x:120,y:720},label:'Moon Shrine Grove',dir:'right'}
      ],
    ...
    shrine: {
      ...
      portals:[{id:'to-village',x:20,y:610,w:120,h:300,to:'village',spawn:{x:1600,y:690},label:'Bramblewick Village',dir:'left'}],
  ...
  function ribbonName(r){ return r.display || r.name || (r.id === 'shrine' ? 'Moon Ribbon' : r.id); }
  ...
  function showToast(text,seconds=2.2){
    toastEl.textContent=text; toastEl.classList.remove('hidden'); toastTimer=seconds;
  }
  ...
  function advanceDialogue(){
    ...
    speakerEl.textContent=currentDialogue.speaker; dialogueText.textContent=currentDialogue.text;
  ...
  function chirp(freq=440,duration=.05){
    ...
    try{
      ...
      o.type='sine'; o.frequency.value=freq; g.gain.setValueAtTime(.032,audioCtx.currentTime);
  ...
  function blockedAt(x,y){
    ...
    if(m.river){
      const inRiver=x>m.river.x-r&&x<m.river.x+m.river.w+r;
      const onBridge=y>m.river.bridgeY+r*.1&&y<m.river.bridgeY+m.river.bridgeH-r*.1;
  ...
  function movePlayer(dx,dy,dt){
    ...
    player.walkT+=dt*(sprinting?13:8);
  ...
  function checkPortal(){
    ...
    for(const p of m.portals){
      if(player.x>=p.x&&player.x<=p.x+p.w&&player.y>=p.y&&player.y<=p.y+p.h){
        transitionTo(p.to,p.spawn,p.label); return;
      }
    }
  }
  ...
  function transitionTo(mapId,spawn,label){
    ...
    transitionBusy=true; paused=true; transitionCooldown=.9;
    const target=MAPS[mapId];
    mapTransitionTitle.textContent=label||target.name; mapTransitionSubtitle.textContent=target.subtitle;
    mapTransition.classList.add('show'); chirp(240,.12);
    setTimeout(()=>{
      player.map=mapId; player.x=spawn.x; player.y=spawn.y; state.visited[mapId]=true;
      camera.x=clamp(player.x-VIEW.w/2,0,Math.max(0,target.w-VIEW.w));
      camera.y=clamp(player.y-VIEW.h/2,0,Math.max(0,target.h-VIEW.h));
      updateHud(); saveGame();
    },220);
  ...
  function interact(){
    ...
    const m=currentMap(), candidates=[];
    m.npcs.forEach(n=>candidates.push({kind:'npc',obj:n,d:dist(player.x,player.y,n.x,n.y)}));
    m.signs.forEach(s=>candidates.push({kind:'sign',obj:s,d:dist(player.x,player.y,s.x,s.y)}));
    if(m.lever) candidates.push({kind:'lever',obj:m.lever,d:dist(player.x,player.y,m.lever.x,m.lever.y)});
    if(m.shrine) candidates.push({kind:'shrine',obj:m.shrine,d:dist(player.x,player.y,m.shrine.x,m.shrine.y)});
    const best=candidates.filter(c=>c.d<95).sort((a,b)=>a.d-b.d)[0];
    if(!best){ showToast('Nothing here but wind, grass, and one very busy fox nose.',1.4); return; }
    if(best.kind==='sign') showDialogue('Wooden Sign',[best.obj.text]);
    if(best.kind==='lever'){
      if(!state.gateOpen){ state.gateOpen=true; saveGame(); showToast('The bridge gate clunks open.'); chirp(180,.14); showDialogue('Saffron',['Ha. Opposable thumbs remain wildly overrated.']); }
      else showDialogue('Saffron',['Already open. I remain undefeated by basic machinery.']);
    }
  ...
  function talkNpc(npc){
    if(npc.id==='rowan'){
      if(player.questStage===0){
        showDialogue('Elder Rowan',[
          'The storm scattered your ribbons farther than I feared. One stayed in Bramblewick, one vanished into Mosswood, and one crossed into the Moon Shrine Grove.',
          'Follow the glowing exits when scent vision is active. The paths now lead to separate places instead of one endless stretch of ground.',
          'Bring all three ribbons to the shrine. And yes, the bridge lever still counts as dignified fox engineering.'
        ],()=>{player.questStage=1;updateHud();saveGame();});
      } else if(player.ribbons.length<3) showDialogue('Elder Rowan',[`Your tail inventory says ${player.ribbons.length}/3. The trails between maps are part of the hunt now.`]);
      else showDialogue('Elder Rowan',['All three. Good. Take the eastern trail to Moon Shrine Grove and finish what the storm started.']);
      return;
    }
    ...
    if(npc.id==='ink') showDialogue('Ink the Crow',[player.ribbons.includes('shrine')?'The blue ribbon is yours now. Tragic. It improved the scenery.':'Blue light fell south of the shrine. I saw it first, which means this information is technically mine.']);
  }
  ...
  function interactShrine(){
    ...
    if(player.questStage<3){
      showDialogue('Moon Shrine',[
        'The three ribbons rise from Saffron’s satchel and orbit the carved stone like tiny comets.',
        'Gold, green, and blue settle into the three hollows. The grove exhales, and every path between the maps glimmers at once.',
        'Something under the roots stops listening.'
      ],()=>{
        player.questStage=3; updateHud(); saveGame();
        const s=currentMap().shrine;
        for(let i=0;i<90;i++) particles.push({x:s.x,y:s.y,vx:(Math.random()-.5)*260,vy:(Math.random()-.5)*260,life:1.8,max:1.8,size:3+Math.random()*5,color:['#ffd65a','#78e08f','#9fdcff'][i%3]});
        showToast('MOON SHRINE RESTORED ✦',4); chirp(660,.35);
      });
  ...
  function activateScent(){
    ...
    player.scent=4.3; updateHud(); chirp(520,.1); showToast('Scent vision: trails and exits brighten.',1.4);
  }

  function tailSwish(){
    if(!gameStarted||currentDialogue||player.swish>0) return;
    player.swish=.5; swishWaves.push({map:player.map,x:player.x,y:player.y,r:20,life:.42,max:.42}); chirp(220,.04);
    let harvested=false;
    for(const b of currentMap().bushes){
      if(!state.pickedBerries[b.id]&&dist(player.x,player.y,b.x,b.y)<90){
        state.pickedBerries[b.id]=true; player.berries+=3; harvested=true;
        for(let i=0;i<12;i++) particles.push({x:b.x,y:b.y,vx:(Math.random()-.5)*90,vy:(Math.random()-.5)*90,life:.7,max:.7,size:5,color:'#7f73d8'});
      }
    }
    if(harvested){showToast('+3 moonberries');updateHud();saveGame();}
  }

  function checkCollectibles(){
    for(const r of ribbons){
      if(r.map!==player.map||player.ribbons.includes(r.id)) continue;
      if(dist(player.x,player.y,r.x,r.y)<46){
        player.ribbons.push(r.id);
        ...
        showToast(`${ribbonName(r)} recovered!`,2.7); chirp(760,.2);
  ...
  function update(dt){
    ...
    player.scent=Math.max(0,player.scent-dt); player.swish=Math.max(0,player.swish-dt);
    scentStatus.classList.toggle('active',player.scent>0);
    if(!currentDialogue){
      ...
      movePlayer(dx,dy,dt); checkCollectibles(); checkPortal();
    } else player.moving=false;
    particles.forEach(p=>{p.x+=p.vx*dt;p.y+=p.vy*dt;p.vx*=.985;p.vy*=.985;p.life-=dt;});
    particles=particles.filter(p=>p.life>0);
    swishWaves.forEach(w=>{w.r+=220*dt;w.life-=dt;}); swishWaves=swishWaves.filter(w=>w.life>0);
    clampCamera(false,dt);
  }

  function clampCamera(force=false,dt=.016){
    const m=currentMap();
    const tx=clamp(player.x-VIEW.w/2,0,Math.max(0,m.w-VIEW.w));
    const ty=clamp(player.y-VIEW.h/2,0,Math.max(0,m.h-VIEW.h));
    if(force){camera.x=tx;camera.y=ty;} else {camera.x+=(tx-camera.x)*Math.min(1,dt*7);camera.y+=(ty-camera.y)*Math.min(1,dt*7);}
  }
  ...
  function drawGround(m){
    ctx.fillStyle=player.scent>0?shade(m.ground,-28):m.ground;ctx.fillRect(0,0,m.w,m.h);
    ctx.fillStyle=player.scent>0?shade(m.path,-18):m.path;
    if(player.map==='village'){
      roundedRect(170,650,760,200,85);ctx.fill(); roundedRect(780,690,800,120,50);ctx.fill(); roundedRect(1500,500,190,400,70);ctx.fill(); roundedRect(120,1010,420,180,70);ctx.fill();
    } else if(player.map==='mosswood'){
      roundedRect(90,650,1700,170,80);ctx.fill(); roundedRect(570,320,190,760,80);ctx.fill();
      drawMushrooms();
    } else {
      roundedRect(80,650,1460,190,85);ctx.fill(); roundedRect(720,480,200,650,75);ctx.fill(); roundedRect(1180,320,300,240,80);ctx.fill();
    }
    if(m.river){
      ctx.fillStyle=player.scent>0?'#245a68':m.water;ctx.fillRect(m.river.x,0,m.river.w,m.h);
      for(let y=15;y<m.h;y+=58){ctx.strokeStyle='rgba(255,255,255,.14)';ctx.lineWidth=3;ctx.beginPath();ctx.moveTo(m.river.x+10,y);ctx.quadraticCurveTo(m.river.x+m.river.w/2,y+15,m.river.x+m.river.w-10,y);ctx.stroke();}
      ctx.fillStyle='#725039';ctx.fillRect(m.river.x-10,m.river.bridgeY,m.river.w+20,m.river.bridgeH);
      ctx.fillStyle='#9e7650';for(let x=m.river.x-3;x<m.river.x+m.river.w+5;x+=20)ctx.fillRect(x,m.river.bridgeY+8,13,m.river.bridgeH-16);
    }
  ...
  function shade(hex,amt){
    const n=parseInt(hex.slice(1),16),r=clamp((n>>16)+amt,0,255),g=clamp(((n>>8)&255)+amt,0,255),b=clamp((n&255)+amt,0,255);return `rgb(${r},${g},${b})`;
  }

  function drawMushrooms(){
    for(let i=0;i<18;i++){
      const x=120+((i*173)%1600),y=180+((i*241)%1060);ctx.fillStyle=player.scent>0?'#83d9d9':'#7ba9a5';ctx.beginPath();ctx.arc(x,y,8,Math.PI,0);ctx.fill();ctx.fillStyle='#d8d1b5';ctx.fillRect(x-2,y,4,10);
    }
  }

  function drawHouse(h){
    ctx.fillStyle='rgba(0,0,0,.2)';roundedRect(h.x+14,h.y+66,h.w,h.h-40,18);ctx.fill();ctx.fillStyle=h.wall;roundedRect(h.x,h.y+52,h.w,h.h-52,14);ctx.fill();ctx.fillStyle='#4b3426';ctx.fillRect(h.x+h.w*.43,h.y+h.h-70,50,70);ctx.fillStyle='#9ed7df';ctx.fillRect(h.x+32,h.y+102,48,40);ctx.fillRect(h.x+h.w-80,h.y+102,48,40);ctx.fillStyle=h.roof;ctx.beginPath();ctx.moveTo(h.x-24,h.y+70);ctx.lineTo(h.x+h.w*.5,h.y);ctx.lineTo(h.x+h.w+24,h.y+70);ctx.closePath();ctx.fill();
  }
  ...
  function drawPortals(m,t){
    for(const p of m.portals){
      const cx=p.x+p.w/2,cy=p.y+p.h/2,pulse=.5+.5*Math.sin(t*3+p.x);
      ctx.save();ctx.globalAlpha=.42+pulse*.3;ctx.strokeStyle=player.scent>0?'#bff8ff':'#f6dfa1';ctx.lineWidth=4;ctx.setLineDash([10,10]);ctx.strokeRect(p.x+8,p.y+8,p.w-16,p.h-16);ctx.setLineDash([]);ctx.fillStyle='rgba(16,27,21,.78)';roundedRect(cx-68,cy-18,136,36,18);ctx.fill();ctx.fillStyle='#fff4d8';ctx.font='bold 11px system-ui';ctx.textAlign='center';ctx.fillText(p.label,cx,cy+4);ctx.restore();
    }
  }
  ...
  function drawScentTrails(t){
    ...
    const targets=[];
    for(const r of ribbons){if(r.map===player.map&&!player.ribbons.includes(r.id))targets.push({x:r.x,y:r.y,color:r.color});}
    if(!targets.length&&player.ribbons.length<3){
      const missingMaps=new Set(ribbons.filter(r=>!player.ribbons.includes(r.id)).map(r=>r.map));
      for(const p of currentMap().portals)if(missingMaps.has(p.to))targets.push({x:p.x+p.w/2,y:p.y+p.h/2,color:'#bff8ff'});
    }
    ...
    targets.forEach(target=>{
      const dx=target.x-player.x,dy=target.y-player.y,d=Math.hypot(dx,dy);if(d<1)return;const nx=dx/d,ny=dy/d,count=Math.min(18,Math.floor(d/58));
      for(let i=1;i<=count;i++){const px=player.x+nx*i*56,py=player.y+ny*i*56,pulse=.45+.45*Math.sin(t*5-i*.8);ctx.globalAlpha=.28+pulse*.5;ctx.fillStyle=target.color;ctx.beginPath();ctx.arc(px,py,4+pulse*3,0,Math.PI*2);ctx.fill();}
    });ctx.globalAlpha=1;
  }

  function drawFox(t){
    ctx.save();ctx.translate(player.x,player.y);const bob=player.moving?Math.sin(player.walkT)*2:Math.sin(t*2)*.5;ctx.translate(0,bob);
    ctx.fillStyle='rgba(0,0,0,.2)';ctx.beginPath();ctx.ellipse(0,22,34,11,0,0,Math.PI*2);ctx.fill();
    if(player.dir==='left'||player.dir==='right') drawFoxSide(player.dir==='left'?-1:1,t);
    else drawFoxVertical(player.dir==='up',t);
    if(player.swish>0){ctx.strokeStyle='rgba(255,231,176,.75)';ctx.lineWidth=5;ctx.beginPath();ctx.arc(0,2,58,-2.5,.7);ctx.stroke();}
    ctx.restore();
  }
  ...
  function drawFoxSide(flip,t){
    ...
    ctx.fillStyle='#d66b32';ctx.beginPath();ctx.ellipse(-40,8+tailWave*.12,40,15,-.1,0,Math.PI*2);ctx.fill();ctx.fillStyle='#f2e4c9';ctx.beginPath();ctx.ellipse(-70,11+tailWave*.12,17,10,-.1,0,Math.PI*2);ctx.fill();
    ctx.fillStyle='#dd7638';ctx.beginPath();ctx.ellipse(0,4,34,20,0,0,Math.PI*2);ctx.fill();ctx.beginPath();ctx.arc(27,-5,21,0,Math.PI*2);ctx.fill();
    ctx.beginPath();ctx.moveTo(18,-21);ctx.lineTo(21,-46);ctx.lineTo(33,-23);ctx.closePath();ctx.fill();ctx.beginPath();ctx.moveTo(32,-22);ctx.lineTo(45,-42);ctx.lineTo(45,-16);ctx.closePath();ctx.fill();
    ctx.fillStyle='#f5ead3';ctx.beginPath();ctx.moveTo(32,4);ctx.lineTo(52,3);ctx.lineTo(34,15);ctx.lineTo(19,13);ctx.closePath();ctx.fill();ctx.fillStyle='#1e1c19';ctx.beginPath();ctx.arc(49,2,4,0,Math.PI*2);ctx.fill();ctx.beginPath();ctx.arc(34,-8,2.6,0,Math.PI*2);ctx.fill();
    const step=player.moving?Math.sin(player.walkT)*4:0;ctx.fillStyle='#3a2c24';ctx.fillRect(-11,15,8,18+step);ctx.fillRect(14,15,8,18-step);ctx.restore();
  }

  function drawFoxVertical(up,t){
    const step=player.moving?Math.sin(player.walkT)*4:0;
    ctx.fillStyle='#d66b32';ctx.beginPath();ctx.ellipse(0,23,18,42,0,0,Math.PI*2);ctx.fill();ctx.fillStyle='#f2e4c9';ctx.beginPath();ctx.ellipse(0,58,11,19,0,0,Math.PI*2);ctx.fill();
    ctx.fillStyle='#dd7638';ctx.beginPath();ctx.ellipse(0,0,25,30,0,0,Math.PI*2);ctx.fill();
    if(up){ctx.beginPath();ctx.moveTo(-18,-17);ctx.lineTo(-14,-43);ctx.lineTo(-4,-20);ctx.closePath();ctx.fill();ctx.beginPath();ctx.moveTo(18,-17);ctx.lineTo(14,-43);ctx.lineTo(4,-20);ctx.closePath();ctx.fill();ctx.fillStyle='#7c3e2d';ctx.beginPath();ctx.arc(0,-10,5,0,Math.PI*2);ctx.fill();}
    else{ctx.beginPath();ctx.moveTo(-18,-16);ctx.lineTo(-14,-42);ctx.lineTo(-4,-19);ctx.closePath();ctx.fill();ctx.beginPath();ctx.moveTo(18,-16);ctx.lineTo(14,-42);ctx.lineTo(4,-19);ctx.closePath();ctx.fill();ctx.fillStyle='#f5ead3';ctx.beginPath();ctx.ellipse(0,4,17,15,0,0,Math.PI*2);ctx.fill();ctx.fillStyle='#1e1c19';ctx.beginPath();ctx.arc(-8,-7,2.5,0,Math.PI*2);ctx.arc(8,-7,2.5,0,Math.PI*2);ctx.arc(0,6,3.5,0,Math.PI*2);ctx.fill();}
    ctx.fillStyle='#3a2c24';ctx.fillRect(-18,17,7,17+step);ctx.fillRect(11,17,7,17-step);
  }
  ...
  function drawNearbyHint(){
    if(currentDialogue)return;const m=currentMap();let nearest=Infinity;for(const o of [...m.npcs,...m.signs])nearest=Math.min(nearest,dist(player.x,player.y,o.x,o.y));if(m.lever)nearest=Math.min(nearest,dist(player.x,player.y,m.lever.x,m.lever.y));if(m.shrine)nearest=Math.min(nearest,dist(player.x,player.y,m.shrine.x,m.shrine.y));
    if(nearest<92){ctx.save();ctx.translate(player.x,player.y-58);ctx.fillStyle='rgba(15,20,16,.85)';roundedRect(-48,-15,96,30,15);ctx.fill();ctx.fillStyle='#fff4d8';ctx.font='bold 12px system-ui';ctx.textAlign='center';ctx.fillText(isTouch?'USE':'E  INTERACT',0,5);ctx.restore();}
  }

  function draw(){
    const t=performance.now()/1000,m=currentMap();ctx.setTransform(DPR,0,0,DPR,0,0);ctx.clearRect(0,0,VIEW.w,VIEW.h);ctx.save();ctx.translate(-camera.x,-camera.y);drawGround(m);drawScentTrails(t);m.houses.forEach(drawHouse);m.trees.forEach(drawTree);m.bushes.forEach(drawBush);m.signs.forEach(drawSign);drawLever(m);drawShrine(m,t);drawPortals(m,t);ribbons.forEach(r=>drawRibbon(r,t));m.npcs.forEach(drawNpc);drawParticles();drawFox(t);drawNearbyHint();ctx.restore();
    if(player.scent>0){const g=ctx.createRadialGradient(VIEW.w/2,VIEW.h/2,80,VIEW.w/2,VIEW.h/2,Math.max(VIEW.w,VIEW.h)*.7);g.addColorStop(0,'rgba(77,172,181,0)');g.addColorStop(1,'rgba(15,48,53,.34)');ctx.fillStyle=g;ctx.fillRect(0,0,VIEW.w,VIEW.h);}
  }
  ...
  document.querySelectorAll('[data-touch]').forEach(btn=>{
    ...
    const down=e=>{e.preventDefault();if(['up','down','left','right','sprint'].includes(action)){touch[action]=true;btn.classList.add('pressed');}else if(action==='interact')interact();else if(action==='scent')activateScent();else if(action==='swish')tailSwish();};
    const up=e=>{e.preventDefault();if(['up','down','left','right','sprint'].includes(action)){touch[action]=false;btn.classList.remove('pressed');}};
    btn.addEventListener('pointerdown',down);btn.addEventListener('pointerup',up);btn.addEventListener('pointercancel',up);btn.addEventListener('pointerleave',up);
  });

  dialogueBox.addEventListener('pointerdown',()=>{if(currentDialogue)advanceDialogue();});
  startBtn.addEventListener('click',()=>begin(false));continueBtn.addEventListener('click',()=>begin(true));continueBtn.style.display=(localStorage.getItem(SAVE_KEY)||localStorage.getItem(OLD_SAVE_KEY))?'block':'none';
  muteBtn.addEventListener('click',()=>{muted=!muted;muteBtn.textContent=muted?'🔇':'🔊';});window.addEventListener('beforeunload',saveGame);
  document.addEventListener('contextmenu',e=>{if(isTouch)e.preventDefault();});
